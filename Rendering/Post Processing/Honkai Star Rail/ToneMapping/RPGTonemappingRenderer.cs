...
namespace HoyoToon.Rendering.PostProcessing.HSR.ToneMapping
{
    public class RPGTonemappingRenderer : ScriptableRendererFeature
    {
        ...
        private RPGTonemappingPass _tonemappingPass;
        ...
        {
            _tonemappingPass = new RPGTonemappingPass();
        }
        ...
        class RPGTonemappingPass : ScriptableRenderPass
        {
            ...
            {
                _tonemappingMaterial = CoreUtils.CreateEngineMaterial(TonemappingShaderName);
            ...
            {
                if (_tonemappingPassIndex != int.MinValue)
                {
                    return _tonemappingPassIndex;
                }

                _tonemappingPassIndex = _tonemappingMaterial != null ? _tonemappingMaterial.FindPass(TonemappingPassName) : -1;
                if (_tonemappingPassIndex < 0)
                {
                    Debug.LogWarning($"{nameof(RPGTonemappingRenderer)}: shader pass '{TonemappingPassName}' was not found on '{TonemappingShaderName}'.");
                }
            ...
            private bool TryApplyVolumeSettings()
            {
                ...
                _tonemappingMaterial.SetFloat(_levelHighToneId, uniforms.levelHighTone);
                _tonemappingMaterial.SetFloat(_levelShadowToneId, uniforms.levelShadowTone);
                _tonemappingMaterial.SetVector(_levelColorId, uniforms.levelColor);
                _tonemappingMaterial.SetVector(_colorSaturationShadowId, uniforms.colorSaturationShadow);
                _tonemappingMaterial.SetVector(_colorContrastShadowId, uniforms.colorContrastShadow);
                _tonemappingMaterial.SetVector(_colorGainShadowId, uniforms.colorGainShadow);
                _tonemappingMaterial.SetVector(_colorSaturationMidtoneId, uniforms.colorSaturationMidtone);
                _tonemappingMaterial.SetVector(_colorContrastMidtoneId, uniforms.colorContrastMidtone);
                _tonemappingMaterial.SetVector(_colorGainMidtoneId, uniforms.colorGainMidtone);
                _tonemappingMaterial.SetVector(_colorSaturationHighlightId, uniforms.colorSaturationHighlight);
                _tonemappingMaterial.SetVector(_colorContrastHighlightId, uniforms.colorContrastHighlight);
                _tonemappingMaterial.SetVector(_colorGainHighlightId, uniforms.colorGainHighlight);
                _tonemappingMaterial.SetFloat(_colorCorrectionShadowMaxId, uniforms.colorCorrectionShadowMax);
                _tonemappingMaterial.SetFloat(_colorCorrectionHighlightMinId, uniforms.colorCorrectionHighlightMin);
                _tonemappingMaterial.SetVector(_lut2DParamsId, uniforms.lut2DParams);
                _tonemappingMaterial.SetVector(_hueSatConId, uniforms.hueSatCon);
                _tonemappingMaterial.SetVector(_customToneCurveId, uniforms.customToneCurve);
                _tonemappingMaterial.SetVector(_toeSegmentAId, uniforms.toeSegmentA);
                _tonemappingMaterial.SetVector(_toeSegmentBId, uniforms.toeSegmentB);
                _tonemappingMaterial.SetVector(_midSegmentAId, uniforms.midSegmentA);
                _tonemappingMaterial.SetVector(_midSegmentBId, uniforms.midSegmentB);
                _tonemappingMaterial.SetVector(_shoSegmentAId, uniforms.shoSegmentA);
                _tonemappingMaterial.SetVector(_shoSegmentBId, uniforms.shoSegmentB);
                _tonemappingMaterial.SetFloat(_expandGamutId, uniforms.expandGamut);
                _tonemappingMaterial.SetFloat(_hdrHeadroomId, uniforms.hdrHeadroom);
                _tonemappingMaterial.SetFloat(_enableHdrTonemappingId, uniforms.enableHdrTonemapping);
                _tonemappingMaterial.SetFloat(_debugHdrOutputIntermediateId, uniforms.debugHdrOutputIntermediate);
                _tonemappingMaterial.SetFloat(_forceDisableToneMappingId, uniforms.forceDisableToneMapping);
