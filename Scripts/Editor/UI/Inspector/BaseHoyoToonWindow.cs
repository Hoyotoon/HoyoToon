...
namespace HoyoToon.Editor.UI.Windows
{
    public abstract class BaseHoyoToonWindow : EditorWindow
    {
        protected static class UiLayout
        {
            ...
            public const float PADDING_TOP = 8f;
            public const float PADDING_MID1 = 4f;
            public const float PADDING_MID2 = 6f;
            public const float PADDING_BOTTOM = 15f;
        ...
        public class TopBarConfig
        {
            ...
            public string LogoResourcePath;
        ...
        private Texture2D _topBarBackgroundTexture;
        private Texture2D _topBarLogoTexture;
        ...
        protected virtual void OnEnable()
        {
            ...
            CacheTopBarTextures(_topBar);
            _pendingResize = true;
        }

        private static Texture2D LoadTopBarTexture(string path)
        {
            if (string.IsNullOrEmpty(path)) return null;
            return Resources.Load<Texture2D>(path);
        }

        private void CacheTopBarTextures(TopBarConfig cfg)
        {
            var resolved = cfg ?? TopBarConfig.Default();
            _topBarBackgroundTexture = LoadTopBarTexture(resolved.BackgroundResourcePath);
            _topBarLogoTexture = LoadTopBarTexture(resolved.LogoResourcePath);
        }
        ...
        private void ComputeLayoutMeasurements()
        {
            ...
            float headerTextWidth = _cachedContentWidth - 24f;
        ...
        private float ComputeDynamicMinWindowHeight(float headerHeight)
        {
            float headerH = headerHeight > 0f ? headerHeight : 20f;
            ...
            return TopBarLayout.BANNER_HEIGHT
        ...
        private void ApplyDynamicMinHeight(float headerHeight)
        {
            float dynamicMin = ComputeDynamicMinWindowHeight(headerHeight);
            minSize = new Vector2(
                Mathf.Max(UiLayout.WINDOW_MIN_WIDTH, minSize.x),
                Mathf.Max(dynamicMin, minSize.y)
            );
        }
        ...
        private void MaybeAutoResizeToContent()
        {
            ...
            if (_pendingResize && e.type == EventType.Repaint)
            {
                ...
                float dynamicMin = ComputeDynamicMinWindowHeight(_cachedHeaderHeight);
                // Keep footer/actions visible on resize by clamping against the dynamic non-card minimum.
                ...
                ApplyDynamicMinHeight(_cachedHeaderHeight);
        ...
        protected virtual void PreSizeBeforeShow(float initialWidth)
        {
            try
            {
                ...
                float dynamicMin = ComputeDynamicMinWindowHeight(headerH);
                // During initial sizing we keep CARD_MIN_VIEW_HEIGHT as a comfort target, not a hard invariant.
                float newH = Mathf.Clamp(desired, dynamicMin, UiLayout.WINDOW_MAX_HEIGHT);
        ...
        private void DrawTopBar()
        {
            ...
            if (_topBarBackgroundTexture)
            {
                GUI.DrawTexture(data.bgRect, _topBarBackgroundTexture, ScaleMode.StretchToFill);
            }

            if (_topBarLogoTexture)
            {
                GUI.DrawTexture(data.logoRect, _topBarLogoTexture, ScaleMode.ScaleToFit);
            }
        }
        ...
        // Cached style for progress bar percentage text (avoids allocating every frame).
        private GUIStyle _progressPercentStyle;
        ...
        protected virtual void DrawProgressBar()
        {
            using (new EditorGUILayout.VerticalScope())
            {
                ...
                if (_progressPercentStyle == null)
                {
                    _progressPercentStyle = new GUIStyle(EditorStyles.miniLabel)
                    {
                ...
                GUI.Label(progressRect, percentText, _progressPercentStyle);
            }
        }
        ...
        protected virtual void CloseWithResult(int index)
        {
