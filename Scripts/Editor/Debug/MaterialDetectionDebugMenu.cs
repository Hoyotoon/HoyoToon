            ...
            foreach (var obj in selected)
            {
                ...
                if (path.EndsWith(".json", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (MaterialDetection.TryDetectFromJsonFile(abs, out var gameKey, out var shaderPath, out var shader, out var reason))
                    {
                        ...
                        var character = MaterialDetection.TryExtractCharacterName(gameKey, abs);
                        var charLog = string.IsNullOrEmpty(character) ? string.Empty : $", Character='{character}'";
                        HoyoToonLogger.APIInfo($"Detected: Game='{gameKey}', Shader='{shaderPath}' ({shaderState}){charLog}");
                        ...
                        if (!string.IsNullOrEmpty(character)) report.AppendLine($"  - Character: {character}");
                ...
                {
                    ok++;
                    var character = MaterialDetection.TryExtractCharacterName(game, path);
                    var charLog = string.IsNullOrEmpty(character) ? string.Empty : $", Character='{character}'";
                    HoyoToonLogger.APIInfo($"Detected (context): Game='{game}'{charLog}");
        ...
        private static void Menu_DetectForSelection_AllJsons()
        {
            ...
            foreach (var obj in selected)
            {
                ...
                foreach (var (game, shaderPath, src) in results)
                {
                    ...
                    if (!string.IsNullOrEmpty(game)) character = MaterialDetection.TryExtractCharacterName(game, path);
