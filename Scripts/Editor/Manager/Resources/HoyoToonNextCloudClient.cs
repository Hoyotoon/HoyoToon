...
namespace HoyoToon
{
    ...
    public static class HoyoToonNextCloudClient
    {
        ...
        public static async Task<List<RemoteFileInfo>> GetFileListAsync(string shareUrl)
        {
            ...
            HoyoToonLogs.LogDebug($"🔍 Starting comprehensive file discovery for: {shareUrl}");
            HoyoToonLogs.LogDebug($"📋 Extracted token: {token}");

            using (var httpClient = new HttpClient())
            {
                httpClient.Timeout = TimeSpan.FromMinutes(10); // Increased for recursive traversal
                
                // Method 1: Try WebDAV PROPFIND
                try
                {
                    HoyoToonLogs.LogDebug("🌐 Attempting WebDAV PROPFIND...");
                    files = await TryWebDavPropFind(httpClient, shareUrl, token);
                    if (files.Any())
                    {
                        HoyoToonLogs.LogDebug($"✅ WebDAV PROPFIND successful: {files.Count} files found");
                        return files;
                    }
                    else
                    {
                        HoyoToonLogs.LogDebug("❌ WebDAV PROPFIND returned no files");
                    }
                }
                catch (Exception ex)
                {
                    HoyoToonLogs.LogDebug($"❌ WebDAV PROPFIND failed: {ex.Message}");
                }

                // Method 2: Try NextCloud OCS API
                try
                {
                    HoyoToonLogs.LogDebug("🔧 Attempting NextCloud OCS API...");
                    files = await TryOcsApi(httpClient, shareUrl, token);
                    if (files.Any())
                    {
                        HoyoToonLogs.LogDebug($"✅ OCS API successful: {files.Count} files found");
                        return files;
                    }
                    else
                    {
                        HoyoToonLogs.LogDebug("❌ OCS API returned no files");
                    }
                }
                catch (Exception ex)
                {
                    HoyoToonLogs.LogDebug($"❌ OCS API failed: {ex.Message}");
                }

                // Method 3: Try parsing the share page HTML with recursive traversal
                try
                {
                    HoyoToonLogs.LogDebug("📄 Attempting HTML parsing with recursive traversal...");
                    files = await TryHtmlParsing(httpClient, shareUrl, token);
                    if (files.Any())
                    {
                        HoyoToonLogs.LogDebug($"✅ HTML parsing successful: {files.Count} files found");
                        return files;
                    }
                    else
                    {
                        HoyoToonLogs.LogDebug("❌ HTML parsing returned no files");
                    }
                }
                catch (Exception ex)
                {
                    HoyoToonLogs.LogDebug($"❌ HTML parsing failed: {ex.Message}");
                }

                // Method 4: Enhanced manual discovery
                try
                {
                    HoyoToonLogs.LogDebug("🔍 Attempting enhanced manual discovery...");
                    files = await TryManualDiscovery(httpClient, shareUrl, token);
                    if (files.Any())
                    {
                        HoyoToonLogs.LogDebug($"✅ Manual discovery successful: {files.Count} files found");
                        return files;
                    }
                    else
                    {
                        HoyoToonLogs.LogDebug("❌ Manual discovery returned no files");
                    }
                }
                catch (Exception ex)
                {
                    HoyoToonLogs.LogDebug($"❌ Manual discovery failed: {ex.Message}");
                }
            }

            var errorMessage = $"❌ All file discovery methods failed for NextCloud share: {shareUrl}";
            HoyoToonLogs.ErrorDebug(errorMessage);
                ...
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (content.StartsWith("{"))
                        {
                            ...
                            HoyoToonLogs.LogDebug($"OCS API response from {apiUrl}: {content}");
                        }
                    }
                }
                catch (Exception ex)
                {
                    HoyoToonLogs.LogDebug($"OCS API endpoint {apiUrl} failed: {ex.Message}");
                }
            }
        ...
        private static async Task<List<RemoteFileInfo>> TryHtmlParsing(HttpClient httpClient, string shareUrl, string token)
        {
            ...
            HoyoToonLogs.LogDebug($"Starting HTML parsing with recursive directory traversal for {shareUrl}");
            ...
            if (jsonStart >= 0)
            {
                ...
                foreach (var line in lines)
                {
                    ...
                    {
                        HoyoToonLogs.LogDebug($"Found potential file data in HTML: {line.Trim()}");
                    }
                }
            }
            ...
            HoyoToonLogs.LogDebug($"HTML parsing completed, found {files.Count} total files");
            return files;
        }
        ...
        private static async Task<List<RemoteFileInfo>> TryManualDiscovery(HttpClient httpClient, string shareUrl, string token)
        {
            ...
            HoyoToonLogs.LogDebug($"Starting enhanced manual discovery for {shareUrl}");
            ...
            try
            {
                ...
                foreach (var url in urlPatterns)
                {
                    try
                    {
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            if (discoveredFiles.Any())
                            {
                                ...
                                HoyoToonLogs.LogDebug($"Manual discovery found {discoveredFiles.Count} files using URL pattern: {url}");
                    ...
                    {
                        HoyoToonLogs.LogDebug($"Manual discovery URL pattern {url} failed: {ex.Message}");
                    }
                }
                    ...
                    foreach (var dir in commonDirs)
                    {
                        try
                        {
                            ...
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                HoyoToonLogs.LogDebug($"Manual discovery found {dirFiles.Count} files in directory: {dir}");
                            }
                        }
                        catch (Exception ex)
                        {
                            HoyoToonLogs.LogDebug($"Manual discovery for directory {dir} failed: {ex.Message}");
                        }
                    }
                }
            }
            ...
            {
                HoyoToonLogs.LogDebug($"Enhanced manual discovery failed: {ex.Message}");
            }

            HoyoToonLogs.LogDebug($"Manual discovery completed, found {files.Count} total files");
            return files;
        }
        ...
        private static async Task<List<RemoteFileInfo>> ParseFileLinksFromHtmlAsync(HttpClient httpClient, string htmlContent, string baseUrl, string token, string currentPath = "")
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (line.Contains("href=") && IsResourceFile(line))
                {
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(href) && !href.StartsWith("http") && !href.StartsWith("#") && href != "../")
                        {
                            ...
                            if (isDir)
                            {
                                ...
                                HoyoToonLogs.LogDebug($"Found directory: {fullPath}");
                            }
                            else
                            {
                                // Add file
                                ...
                                {
                                    HoyoToonLogs.LogDebug($"Decoded HTML file path: '{originalHref}' -> '{decodedHref}' in '{currentPath}'");
                                }
                            }
                        }
                    }
                    ...
                    {
                        HoyoToonLogs.LogDebug($"Error parsing link from line: {ex.Message}");
                    }
                }
            }
            ...
            foreach (var directory in directories)
            {
                try
                {
                    ...
                    HoyoToonLogs.LogDebug($"Exploring directory: {directoryUrl}");
                    ...
                    {
                        HoyoToonLogs.LogDebug($"Failed to access directory {directoryUrl}: {response.StatusCode}");
                    }
                }
                catch (Exception ex)
                {
                    HoyoToonLogs.LogDebug($"Error exploring directory {directory}: {ex.Message}");
                }
            }
        ...
        private static List<RemoteFileInfo> ParseWebDavResponse(string xmlContent, string webdavUrl, string shareUrl, string token)
        {
            ...
            try
            {
                ...
                foreach (var response in doc.Descendants(ns + "response"))
                {
                    ...
                    if (!string.IsNullOrEmpty(relativePath) && relativePath != "/" && !isDirectory)
                    {
                        ...
                        {
                            HoyoToonLogs.LogDebug($"Decoded file path: '{originalPath}' -> '{relativePath}'");
                        }
                    }
                }
            }
            ...
            {
                HoyoToonLogs.ErrorDebug($"Failed to parse WebDAV response: {ex.Message}");
                throw;
            }
