...
{
    internal static class TourOverlay
    {
        ...
        private static readonly Type s_inspectorWindowType = typeof(UnityEditor.Editor).Assembly.GetType("UnityEditor.InspectorWindow");
        private const double RepaintIntervalSeconds = 0.05d;
        ...
        {
            if (!GuidedTourController.IsTargetActive(targetId))
            {
                return;
            }
        ...
        private static void HandleEditorUpdate()
        {
            ...
            s_nextRepaintTime = now + RepaintIntervalSeconds;

            // Only repaint Inspector windows where tour highlights are drawn
            if (s_inspectorWindowType != null)
            {
                foreach (var w in Resources.FindObjectsOfTypeAll<EditorWindow>())
                {
                    if (s_inspectorWindowType.IsInstanceOfType(w))
                        w.Repaint();
                }
            }
        }
