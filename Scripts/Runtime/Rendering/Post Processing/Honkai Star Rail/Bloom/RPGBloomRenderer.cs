...
namespace HoyoToon.Runtime.Rendering.PostProcessing.HSR.Bloom
{
    public class RPGBloomRenderer : ScriptableRendererFeature
    {
        ...
        class RPGBloomRenderPass : ScriptableRenderPass
        {
            ...
            private bool TryApplyVolumeSettings()
            {
                ...
                int kernelSourceHash = 17;
                kernelSourceHash = kernelSourceHash * 31 + _blurStageCount;
                for (int i = 0; i < _blurStageCount; i++)
                {
                    ...
                    float kernelSigma = Mathf.Max(stage.KernalSigma, 1e-5f);
                    _kernelSizes[i] = kernelSize;
                    _kernelSigmas[i] = kernelSigma;
                    _layerIntensities[i] = Mathf.Max(0f, stage.LayerIntensity);
                    kernelSourceHash = kernelSourceHash * 31 + kernelSize;
                    kernelSourceHash = kernelSourceHash * 31 + kernelSigma.GetHashCode();
                }

                if (!_hasBloomKernelData || kernelSourceHash != _lastBloomKernelSourceHash)
                {
                    for (int i = 0; i < _blurStageCount; i++)
                    {
                        CalculateBloomKernel(_kernels[i], _kernelSizes[i]);
                    }

                    _lastBloomKernelSourceHash = kernelSourceHash;
                    _hasBloomKernelData = true;
                    _hasGaussianKernelData = false;
                    _lastGaussianKernelDataHash = 0;
                }
            ...
            {
                int layoutHash = ComputeGaussianKernelDataHash(atlasWidth, atlasHeight);
                if (_hasGaussianKernelData && layoutHash == _lastGaussianKernelDataHash)
                    return;
                ...
                for (int i = 0; i < _blurStageCount; i++)
                {
                    ...
                    CalculateGaussianKernel(
                        ...
                        stageHeight,
                        true,
                        _gaussWeightsHorizontal[i],
                        _gaussOffsetsHorizontal[i],
                        _gaussianWeightScratch);
                    ...
                    CalculateGaussianKernel(
                        ...
                        false,
                        _gaussWeightsVertical[i],
                        _gaussOffsetsVertical[i],
                        _gaussianWeightScratch);
                }

                _lastGaussianKernelDataHash = layoutHash;
                _hasGaussianKernelData = true;
            }

            private int ComputeGaussianKernelDataHash(float atlasWidth, float atlasHeight)
            {
                unchecked
                {
                    int hash = 17;
                    hash = hash * 31 + _blurStageCount;
                    hash = hash * 31 + Mathf.RoundToInt(atlasWidth).GetHashCode();
                    hash = hash * 31 + Mathf.RoundToInt(atlasHeight).GetHashCode();
                    for (int i = 0; i < _blurStageCount; i++)
                    {
                        hash = hash * 31 + _kernelSizes[i];
                        hash = hash * 31 + _kernelSigmas[i].GetHashCode();
                        hash = hash * 31 + Mathf.RoundToInt(_atlasViewports[i].width).GetHashCode();
                        hash = hash * 31 + Mathf.RoundToInt(_atlasViewports[i].height).GetHashCode();
                    }
                ...
                float[] weightsScratch)
            {
                ...
                float totalSum = 0f;
                float[] weights = weightsScratch ?? outWeights;
                ...
                if (totalSum <= 1e-8f)
                {
                    ...
                    for (int i = taps; i < BloomMaxKernelSize; ++i)
                    {
                        outWeights[i] = 0f;
                        outOffsets[i] = Vector4.zero;
                    }
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                for (int i = 0; i < _mipDownCount; i++)
                {
                    ...
                    _mipDownHandles[i] = UniversalRenderer.CreateRenderGraphTexture(renderGraph, mipDesc, $"RPG_BloomMipDown_{i}", false, FilterMode.Bilinear);
                }
