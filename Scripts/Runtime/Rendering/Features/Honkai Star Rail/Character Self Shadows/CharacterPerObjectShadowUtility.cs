...
{
    internal static class CharacterPerObjectShadowUtility
    {
        ...
        public static bool TryBuildSelfShadowMatrices(
            Camera camera,
            Transform casterTransform,
            Bounds bounds,
            Light shadowLight,
            float lightFollowAmount,
            bool invertLightDirection,
            float nearPlane,
            float farPlane,
            ...
            out Vector4 lightDirection)
        {
            ...
            if (camera == null || shadowLight == null)
                return false;
            ...
            Vector3 viewForward = toCenter.normalized;
            Vector3 lightForward;
            if (shadowLight.type == LightType.Directional)
            {
                lightForward = shadowLight.transform.forward;
            }
            else
            {
                lightForward = bounds.center - shadowLight.transform.position;
            ...
            float follow = Mathf.Clamp01(lightFollowAmount);

            // Blend toward the camera-facing direction to reduce back-face instability.
            Vector3 forward = Vector3.Slerp(viewForward, lightForward.normalized, follow);
            ...
            Quaternion lightRotation = Quaternion.LookRotation(forward, camera.transform.up);
            ...
            if (!AdjustViewSpaceShadowAabb(camera, viewMatrix, ref shadowMin, ref shadowMax))
                return false;

            // Ensure the whole caster is in front of the configured near plane.
            // Without this, the light view origin can sit inside the model and cause severe near clipping.
            ...
            float distanceScore = Mathf.Clamp01(toCenter.sqrMagnitude / 10000f);
            float viewAlignment = Vector3.Dot(camera.transform.forward, viewForward);
            priority = distanceScore + (1f - viewAlignment) * 0.5f;
            return true;
        }
    }
}
