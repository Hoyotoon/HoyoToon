vertex_output base_vertex (vertex_input v)
{
    ...
    if(_UseFakeDirectionalLight)
    {
        light = mul((float3x3)unity_ObjectToWorld, _FakeDirectionalLightRotation);
    }

    o.hairpos;
    float3 vl = mul(light, UNITY_MATRIX_V) * (1.f / o.ws_pos.w);
...
float4 base_pixel (vertex_output i, bool vface : SV_IsFrontFace) : SV_Target
{
    ...
    output.xyz = main_tex * color;
    #if defined(is_transparent)
    output.a = main_tex.a * color.a;
    #endif
    ...
    // shadow 
        ...
        selfshadow = smoothstep(0.f, 1.f, selfshadow);;
        shadow_area = shadow_rate(ndotl, lightmap.y, vcol.x, _ShadowRamp, selfshadow);
        ...
        {
            cg_lighting(shadow_color, shadow_area, material_ID);
        }
        if(_ShadowBoost)
        {
    ...
    lighting_color = lighting_color * saturate(shadow_area);
    output.xyz = output.xyz + lighting_color;
    ...
    output.xyz = output.xyz + (specular + lighting_color);
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // rim shadow
        ...
        float sdw_ndotv = ndotv;
        sdw_ndotv = 1.0f - sdw_ndotv;
        sdw_ndotv = max(sdw_ndotv, 0.00100000005);
        sdw_ndotv = saturate(pow(sdw_ndotv, _RimShadowCt));
        sdw_ndotv = smoothstep(rim_shadow_values[ID].y, 1.0f, sdw_ndotv * rim_shadow_values[ID].x);
        sdw_ndotv = ((sdw_ndotv * _RimShadowIntensity) * _ES_RimShadowIntensity) * 0.25f;
        rimsdw_color.xyz = sdw_ndotv * (rimsdw_color * 2 - 1) + 1;
        output.xyz =  rimsdw_color * output;
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // sunglasses, this is only foer the transparent shader
    #if defined(is_transparent)
        if(_Sunglasses)
        {
            float inv_scale_z = 1.0 / _SunGlassesTilingOffset.z;

            float2 scaled_uv = (uv - _SunGlassesTilingOffset.xy) * inv_scale_z;
            scaled_uv = scaled_uv * 2.0 - 1.0; 

            float2 angles_rad = float2(_HighlightAngleL, _HighlightAngleR) * 0.0174532905;

            float2 sin_angles = sin(angles_rad);
            float2 cos_angles = cos(angles_rad);

            // Rotation matrices packed into vectors
            // M_R: (cos_L, sin_L, cos_R, sin_R)
            // M_L: (-sin_L, cos_L, -sin_R, cos_R)
            float4 m_r; 
            float4 m_l; 
            
            m_r.xz = cos_angles.xy;
            m_r.yw = sin_angles.xy; 
            
            m_l.xz = -sin_angles.xy;
            m_l.yw = m_r.xz; 
            
            float4 rotated_vector = scaled_uv.xxxx * m_r + scaled_uv.yyyy * m_l;
            
            float2 rot_l = rotated_vector.xy;
            float2 rot_r = rotated_vector.zw;
            
            float2 blended_rot_v = lerp(rot_l, rot_r, i.color.xx);
            
            float2 shifted_v = blended_rot_v + float2(0.0, 1.0); 

            float r_sq = dot(shifted_v, shifted_v);
            float r_distance = sqrt(r_sq);
            float r_curved = r_distance - 1.0; 
            float radial_value = blended_rot_v.y + _BendValue * (r_curved - blended_rot_v.y);

            float2 inv_total_size = 1.0 / float2(_TotalSizeL, _TotalSizeR);
            float2 wave_input = radial_value.xx * inv_total_size + float2(_HighlightOffsetL, _HighlightOffsetR);
            float2 highlight_wave = sin(wave_input);
            float2 highlight_mask = (highlight_wave + float2(_HighlightWidthL, _HighlightWidthR) - 1.0) 
                                / float2(_BlendRadiusL, _BlendRadiusR);
            highlight_mask = saturate(highlight_mask);

            float final_intensity = lerp(highlight_mask.x, highlight_mask.y, i.color.x);
            float3 final_specular_color = final_intensity * _SunglassesSpecluarColor.xyz * _SunglassesSpecluarColor.w;

            output.xyz = output.xyz + final_specular_color;
    ...
    if(!_UsingDitherAlpha)
    {
        ...
        float3 rim_values[8] = // x = softness, y = type, z = dark
        {
            float3(_RimEdgeSoftness0, _RimType0, saturate(_RimDark0)),
            float3(_RimEdgeSoftness1, _RimType1, saturate(_RimDark1)),
            float3(_RimEdgeSoftness2, _RimType2, saturate(_RimDark2)),
            float3(_RimEdgeSoftness3, _RimType3, saturate(_RimDark3)),
            float3(_RimEdgeSoftness4, _RimType4, saturate(_RimDark4)),
            float3(_RimEdgeSoftness5, _RimType5, saturate(_RimDark5)),
            float3(_RimEdgeSoftness6, _RimType6, saturate(_RimDark6)),
            float3(_RimEdgeSoftness7, _RimType7, saturate(_RimDark7)),
        }; 


        if(_UseMaterialValuesLUT) 
        {    
            rim_color[ID].xyz = lut_rimcol.xyz;
            rim_values[ID].xyz = lut_rimval.yxz; 
        }

        rim_lighting(lightmap.xy, light, selfshadow, i.screenpos, i.pos, view, normal, rim_color[ID].xyz, rim_values[ID].xyz, output);
...
float4 outline_pixel (vertex_output i, bool vface : SV_IsFrontFace) : SV_Target
{
    ...
    if(_UseMaterialValuesLUT) outline_color.xyz = _MaterialValuesPackLUT.Load(float4(material_ID, 2, 0, 0));
