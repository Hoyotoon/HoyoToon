...
void fake_fog(in float4 pos, in float3 view, inout float4 color)
{
    float view_length = length(view);

    float fog_start = exp2(((_FakeFogStartHeight - pos.y) * -_FakeFogHeightFalloff) * 1.45f) * -_FakeFogDensity;
    fog_start = max(1.0f - exp2((view_length * fog_start) * 1.45f), 0.0f);

    float3 fog_color = fog_start * _FakeFogColor.xyz;
    color.xyz = fog_color + color.xyz;
}

void pointlight(in float4 pos, in float4 modelPos, in float atten, in float3 normal, in float3 lightColor, in float intensity, inout float3 color)
{
    float3 ws_pl = mul((float3x3)unity_ObjectToWorld, pos.xyz);
    float3 toLight = normalize(modelPos.xyz - ws_pl);

    float lightLength = length(modelPos.xyz - ws_pl);
    lightLength = lightLength / max(atten, 0.00001f);
    lightLength = pow(max(1.0f - lightLength, 0.0f), 2.0f);

    float lighting = max(dot(normal, toLight), 0.0f) * lightLength;

    color += (lighting * lightColor) * intensity;
}
...
#define POINTLIGHT_CALC(_pos, _index, _normal, _outColor) \
{ \
    \
    float3 __temp_color_accum = 0; \
    pointlight(_pos, _FakePointLight##_index##Pos, _FakePointLight##_index##AttenuationRadius, _normal, _FakePointLight##_index##Color, _FakePointLight##_index##Intensity, __temp_color_accum); \
    _outColor += __temp_color_accum; \
}
...
float shadow_rate(float ndotl, float lightmap_ao, float vertex_ao, float shadow_ramp, float casted)
{
        ...
        float extra = lightmap_ao < 0.8;
        float shadow_thresh = (lightmap_ao + lightmap_ao);
        float shadow_area = saturate(ndotl * 0.5f + 0.5f) ;
        shadow_area =  min(1.0f, dot(shadow_area.xx, shadow_thresh.xx));
        shadow_area = max(0.001f, shadow_area) * 0.85f + 0.15f;
        shadow_area = (shadow_area > shadow_ramp) ? 0.99f : shadow_area;
        shadow_area = shadow_area * extra;
        return shadow_area;
    #else
...
void cg_lighting(inout float3 shadow_color, float shadow_area, int material_id)
{
