    ...
    */
    ...
    float3 rsdw =  1.0f;
        if(_UseMaterialValuesLUT)
        {
            ...
            rsdw_color = _MaterialValuesPackLUT.Load(float4(id, 5, 0u, 0u)).xyz;
            rsdw_param = _MaterialValuesPackLUT.Load(float4(id, 6, 0u, 0u)).xy;
        }
        else
        {
            float3 rimshadowcolor[8] =
            {
                _RimShadowColor0,
                _RimShadowColor1,
                _RimShadowColor2,
            ...
            rsdw_color = rimshadowcolor[array_index].xyz; 
            rsdw_param = rim_params[array_index];
        }
    ...
    specular = smoothstep(specular_thresh - spec_param.y, specular_thresh + spec_param.y, specular) * spec_color * spec_param.z;
        ...
        float3 tmp_color = sdw_combine.xxx * matcap.xyz;
    ...
    #endif 
    float3 sunglasses_spec = 0.0f;
    #if defined(_SUNGLASSES)
        // Adjust UV coordinates by tiling and offset
        float2 adjusted_uv = i.uv.xy - _SunGlassesTilingOffset.xy;
        float uv_scale = 1.0f / _SunGlassesTilingOffset.z;
        adjusted_uv = adjusted_uv * uv_scale;
        
        // Normalize UV to range [-1, 1]
        float2 normalized_uv = adjusted_uv * 2.0f - 1.0f;
        
        // Convert highlight angles from degrees to radians and compute sin/cos
        float2 angles_rad = float2(_HighlightAngleL, _HighlightAngleR) * 0.0174532905f;
        float2 sin_angles = sin(angles_rad);
        float2 cos_angles = cos(angles_rad);
        
        // Build rotation matrices for left and right highlights
        float2x2 rotation_left = float2x2(cos_angles.x, -sin_angles.x, sin_angles.x, cos_angles.x);
        float2x2 rotation_right = float2x2(cos_angles.y, -sin_angles.y, sin_angles.y, cos_angles.y);
        
        // Apply rotations
        float2 rotated_left = mul(rotation_left, normalized_uv);
        float2 rotated_right = mul(rotation_right, normalized_uv);
        
        // Blend between left and right based on vertex color
        float2 highlight_uv = lerp(rotated_left, rotated_right, i.color.x);
        
        // Calculate highlight position with bend effect
        float highlight_distance = length(highlight_uv.xy) - 1.0f;
        float highlight_pos = highlight_distance - highlight_uv.y;
        highlight_pos = _BendValue * highlight_pos + highlight_uv.y;
        
        // Scale and apply offset
        float2 size_inverse = 1.0f / float2(_TotalSizeL, _TotalSizeR);
        float2 highlight_sample = highlight_pos * size_inverse + float2(_HighlightOffsetL, _HighlightOffsetR);
        
        // Apply sinusoidal highlight pattern
        highlight_sample = sin(highlight_sample);
        highlight_sample = highlight_sample + float2(_HighlightWidthL, _HighlightWidthR);
        highlight_sample = highlight_sample - 1.0f;
        
        // Normalize to [0, 1] range based on blend radius
        highlight_sample = saturate(highlight_sample / float2(_BlendRadiusL, _BlendRadiusR));
        
        // Final blend between left and right highlights
        float final_highlight = lerp(highlight_sample.x, highlight_sample.y, i.color.x);
        sunglasses_spec = final_highlight;
        // Apply sunglasses specular color
        sunglasses_spec = final_highlight * _SunglassesSpecluarColor.xyz * _SunglassesSpecluarColor.www;
    ...
    final_color.xyz = specular * final_color + lerp(final_color, diffuse * _EmissionIntensity, 
    emission_thresh);
    final_color.xyz = final_color.xyz * light_color + fresnel;
    final_color.xyz = _ES_AddColor.xyz * final_color + final_color;
    final_color.xyz = final_color + sunglasses_spec;
...
vertex_out vert_edge(vertex_in v, uint vertexID : SV_VertexID)
{
    ApplyComputeSkinningVertex(v, vertexID);
    vertex_out o = (vertex_out)0.f;
...
forward_mask_out frag_edge(vertex_out i,  bool vface : SV_IsFrontFace)
{
    ...
    forward_mask_out forwardOut;
    forwardOut.color = o.forward;
    forwardOut.alphaMask = float4(1,1,1,1);
    return forwardOut;
}

vertex_out vert_shadow(vertex_in v, uint vertexID : SV_VertexID)
{
    ApplyComputeSkinningVertex(v, vertexID);
