...
float4 hair_transparency_pixel (vertex_output i, bool vface : SV_IsFrontFace ) : SV_Target
{
...
#if defined(ENABLE_NYX_OUTLINE_ON)
vertex_output nyx_vertex (vertex_input v)
{
    vertex_output o = (vertex_output)0;
...
float4 mask_pixel(vertex_output i) : SV_Target
{
...
[domain("tri")]
vertex_output domain_base(tess_struct factors, OutputPatch<control_struct, 3> patch, float3 bary : SV_DomainLocation)
{
    ...
    #if defined(ENABLE_NYX_OUTLINE_ON)
    return nyx_vertex(v_in);
    #elif  ENABLE_OUTLINE_ON
    return edge_vertex(v_in);
    #else
...
[domain("tri")]
vertex_output domain_edge(tess_struct factors, OutputPatch<control_struct, 3> patch, float3 bary : SV_DomainLocation)
{
    float u = bary.x; float v = bary.y; float w = bary.z;
    PN_Control_Patch cp = pn_constant_func(patch);
...
[domain("tri")] 
vertex_output domain_nyx(tess_struct factors, OutputPatch<control_struct, 3> patch, float3 bary : SV_DomainLocation)
{
    float u = bary.x; float v = bary.y; float w = bary.z;
    PN_Control_Patch cp = pn_constant_func(patch);
